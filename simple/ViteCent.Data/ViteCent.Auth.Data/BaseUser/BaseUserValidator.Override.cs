...
public partial class BaseUserValidator : AbstractValidator<AddBaseUserArgs>
{
    ...
    private void OverrideValidator(bool validate = false)
    {
        ...
        {
            RuleFor(x => x.Password).NotNull().NotEmpty().When(x => !string.IsNullOrWhiteSpace(x.Password)).WithMessage("密码不能为空");
            RuleFor(x => x.Password).Length(6, 16).When(x => !string.IsNullOrWhiteSpace(x.Password)).WithMessage("密码6-16个字符");
            RuleFor(x => x.Password).Matches(BaseConst.PositiveEnglishUnderline).When(x => !string.IsNullOrWhiteSpace(x.Password)).WithMessage("密码只支持数字、字母、下划线");
        }
        ...
        RuleFor(x => x.Phone).Matches(BaseConst.Mobile).When(x => !string.IsNullOrWhiteSpace(x.Phone)).WithMessage("电话格式错误");
