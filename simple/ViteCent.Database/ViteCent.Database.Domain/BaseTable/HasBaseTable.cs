...
/// </summary>
/// <remarks>
/// 该类用于处理单个数据表信息的判重逻辑，包括：
/// 1. 检查数据表信息编码是否重复
/// 2. 检查数据表信息名称是否重复
/// 3. 支持在更新时排除当前记录的判重
/// </remarks>
/// <param name="logger">日志记录器实例</param>
public class HasBaseTable(
    // 注入日志记录器
    ILogger<HasBaseTable> logger)
    // 继承基类，指定查询参数和返回结果类型
    : BaseDomain<BaseTableEntity>, IRequestHandler<HasBaseTableEntityArgs, BaseResult>
{
    /// <summary>
    /// 数据库名称
    /// </summary>
    ...
    /// 处理数据表信息判重请求
    /// </summary>
    /// <param name="request">包含待检查的数据表信息标识、编码和名称的请求参数</param>
    ...
    /// 3. 执行查询并返回结果
    /// </remarks>
    public async Task<BaseResult> Handle(HasBaseTableEntityArgs request, CancellationToken cancellationToken)
    {
        // 记录方法调用日志，便于追踪和调试
