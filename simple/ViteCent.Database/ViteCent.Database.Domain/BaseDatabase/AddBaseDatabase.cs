...
public class AddBaseDatabase(
    ...
    // 继承基类，指定查询参数和返回结果类型
    : BaseDomain<AddBaseDatabaseEntity>, IRequestHandler<AddBaseDatabaseEntity, BaseResult>
{
    /// <summary>
    /// 数据库名称
    /// </summary>
    ...
    public async Task<BaseResult> Handle(AddBaseDatabaseEntity request, CancellationToken cancellationToken)
    {
        // 记录方法调用日志，便于追踪和调试
        logger.LogInformation("Invoke ViteCent.Database.Domain.BaseDatabase.AddBaseDatabase");
