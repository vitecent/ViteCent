...
// 设置路由前缀
[Route("BaseDatabase")]
public class AddBaseDatabaseList(
    // 注入日志记录器
    ILogger<AddBaseDatabaseList> logger,
    // 注入HTTP上下文访问器
    IHttpContextAccessor httpContextAccessor,
    // 注入中介者
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseListApi<List<AddBaseDatabaseArgs>, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 7. 返回操作结果
    /// </remarks>
    /// <param name="args">批量新增数据库信息的参数</param>
    /// <returns>返回批量新增操作的结果</returns>
    [HttpPost] // 标记为 Post 请求
    // 使用权限验证过滤器，验证用户是否有权限访问该接口
    [TypeFilter(typeof(BaseAuthFilter), Arguments = new object[] { "Database", "BaseDatabase", "Add" })]
    // 设置路由名称
    [Route("AddList")]
    public override async Task<BaseResult> InvokeAsync(List<AddBaseDatabaseArgs> args)
    {
        // 记录方法调用日志，便于追踪和调试
        logger.LogInformation("Invoke ViteCent.Database.Api.BaseDatabase.AddBaseDatabaseList");
        ...
        var logsArgs = new AddBaseLogsArgs()
        {
            ...
            ResourceName = "BaseDatabase",
            OperationId = string.Empty,
            OperationName = "Add",
            Description = "批量新增数据库信息",
            Args = args.ToJson()
        };
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "数据库信息不能为", cancellationToken);

            // 返回操作结果
            return new BaseResult(500, "数据库信息不能为空");
        }
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "数据库信息重复", cancellationToken);
        ...
        {
            // 重写调用方法
            AddBaseDatabase.OverrideInvoke(item, user);
        ...
        // 构建请求参数
        var request = new AddBaseDatabaseListArgs()
        {
            Items = args
        };
