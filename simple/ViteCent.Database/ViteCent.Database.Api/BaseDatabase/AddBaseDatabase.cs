...
public partial class AddBaseDatabase(
    // 注入日志记录器
    ILogger<AddBaseDatabase> logger,
    // 注入HTTP上下文访问器
    IHttpContextAccessor httpContextAccessor,
    // 注入中介者
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseApi<AddBaseDatabaseArgs, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 6. 返回操作结果
    /// </remarks>
    /// <param name="args">新增数据库信息的参数</param>
    ...
    // 设置路由名称
    [Route("Add")]
    public override async Task<BaseResult> InvokeAsync(AddBaseDatabaseArgs args)
    {
        // 记录方法调用日志，便于追踪和调试
        logger.LogInformation("Invoke ViteCent.Database.Api.BaseDatabase.AddBaseDatabase");
