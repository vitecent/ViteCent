...
public partial class EditBaseDatabase(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseApi<EditBaseDatabaseArgs, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 5. 返回操作结果
    /// </remarks>
    /// <param name="args">编辑参数，包含需要更新的数据库信息数据</param>
    /// <returns>返回基础结果对象，标识操作是否成功</returns>
    [HttpPost] // 标记为 Post 请求
    // 权限验证过滤器，验证用户是否有权限访问该接口
    [TypeFilter(typeof(BaseAuthFilter), Arguments = new object[] { "Database", "BaseDatabase", "Edit" })]
    // 设置路由名称
    [Route("Edit")]
    public override async Task<BaseResult> InvokeAsync(EditBaseDatabaseArgs args)
    {
        // 记录方法调用日志，便于追踪和调试，便于追踪和调试
        logger.LogInformation("Invoke ViteCent.Database.Api.BaseDatabase.EditBaseDatabase");
