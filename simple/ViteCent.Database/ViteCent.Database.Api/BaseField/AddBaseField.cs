...
public partial class AddBaseField(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseApi<AddBaseFieldArgs, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 6. 返回操作结果
    /// </remarks>
    /// <param name="args">新增表字段信息的参数</param>
    ...
    // 设置路由名称
    [Route("Add")]
    public override async Task<BaseResult> InvokeAsync(AddBaseFieldArgs args)
    {
        // 记录方法调用日志，便于追踪和调试
        logger.LogInformation("Invoke ViteCent.Database.Api.BaseField.AddBaseField");
