...
// 设置路由前缀
[Route("BaseField")]
public class AddBaseFieldList(
    // 注入日志记录器
    ILogger<AddBaseFieldList> logger,
    // 注入HTTP上下文访问器
    IHttpContextAccessor httpContextAccessor,
    // 注入中介者
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseListApi<List<AddBaseFieldArgs>, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 7. 返回操作结果
    /// </remarks>
    /// <param name="args">批量新增表字段信息的参数</param>
    /// <returns>返回批量新增操作的结果</returns>
    [HttpPost] // 标记为 Post 请求
    // 使用权限验证过滤器，验证用户是否有权限访问该接口
    [TypeFilter(typeof(BaseAuthFilter), Arguments = new object[] { "Database", "BaseField", "Add" })]
    // 设置路由名称
    [Route("AddList")]
    public override async Task<BaseResult> InvokeAsync(List<AddBaseFieldArgs> args)
    {
        // 记录方法调用日志，便于追踪和调试
        logger.LogInformation("Invoke ViteCent.Database.Api.BaseField.AddBaseFieldList");
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "表字段信息不能为", cancellationToken);

            // 返回操作结果
            return new BaseResult(500, "表字段信息不能为空");
        }
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "表字段信息重复", cancellationToken);
        ...
        // 创建数据验证器，用于支持操作的取消
        var validator = new BaseFieldValidator();

        // 验证每个表字段信息的有效性
        foreach (var item in args)
        {
            // 重写调用方法
            AddBaseField.OverrideInvoke(item, user);
        ...
        // 构建请求参数
        var request = new AddBaseFieldListArgs()
        {
            Items = args
        };
