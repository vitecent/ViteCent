...
public class AddUserRestList(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseListApi<List<AddUserRestArgs>, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 7. 返回操作结果
    /// </remarks>
    /// <param name="args">批量新增调休申请的参数</param>
    ...
    // 设置路由名称
    [Route("AddList")]
    public override async Task<BaseResult> InvokeAsync(List<AddUserRestArgs> args)
    {
        // 记录方法调用日志，便于追踪和调试
        ...
        var logsArgs = new AddBaseLogsArgs()
        {
            ...
            ResourceName = "UserRest",
            OperationId = string.Empty,
            OperationName = "Add",
            Description = "批量新增调休申请",
            Args = args.ToJson()
        };
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "调休申请不能为", cancellationToken);
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "调休申请重复", cancellationToken);
        ...
        // 构建请求参数
        var request = new AddUserRestListArgs()
        {
            Items = args
        };
