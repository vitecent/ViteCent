...
public class AddRepairScheduleList(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseListApi<List<AddRepairScheduleArgs>, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 7. 返回操作结果
    /// </remarks>
    /// <param name="args">批量新增补卡申请的参数</param>
    ...
    // 设置路由名称
    [Route("AddList")]
    public override async Task<BaseResult> InvokeAsync(List<AddRepairScheduleArgs> args)
    {
        // 记录方法调用日志，便于追踪和调试
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "补卡申请不能为", cancellationToken);
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "补卡申请重复", cancellationToken);
        ...
        // 构建请求参数
        var request = new AddRepairScheduleListArgs()
        {
            Items = args
        };
