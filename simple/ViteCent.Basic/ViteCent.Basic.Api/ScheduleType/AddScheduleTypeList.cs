    ...
    : BaseListApi<List<AddScheduleTypeArgs>, BaseResult>
{
    ...
    /// 7. 返回操作结果
    /// </remarks>
    /// <param name="args">批量新增基础排班的参数</param>
    ...
    public override async Task<BaseResult> InvokeAsync(List<AddScheduleTypeArgs> args)
    {
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "基础排班不能为", cancellationToken);
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "基础排班重复", cancellationToken);
