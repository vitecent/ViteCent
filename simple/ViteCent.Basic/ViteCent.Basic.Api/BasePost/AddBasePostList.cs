...
public class AddBasePostList(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseListApi<List<AddBasePostArgs>, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 7. 返回操作结果
    /// </remarks>
    /// <param name="args">批量新增职位信息的参数</param>
    /// <returns>返回批量新增操作的结果</returns>
    [HttpPost] // 标记为 Post 请求
    // 使用权限验证过滤器，验证用户是否有权限访问该接口
    [TypeFilter(typeof(BaseAuthFilter), Arguments = new object[] { "Basic", "BasePost", "Add" })]
    // 设置路由名称
    [Route("AddList")]
    public override async Task<BaseResult> InvokeAsync(List<AddBasePostArgs> args)
    {
        // 记录方法调用日志，便于追踪和调试
        logger.LogInformation("Invoke ViteCent.Basic.Api.BasePost.AddBasePostList");
        ...
        var logsArgs = new AddBaseLogsArgs()
        {
            ...
            ResourceName = "BasePost",
            OperationId = string.Empty,
            OperationName = "Add",
            Description = "批量新增职位信息",
            Args = args.ToJson()
        };
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "职位信息不能为", cancellationToken);
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "职位信息重复", cancellationToken);
        ...
        {
            // 重写调用方法
            AddBasePost.OverrideInvoke(item, user);
        ...
        // 构建请求参数
        var request = new AddBasePostListArgs()
        {
            Items = args
        };
