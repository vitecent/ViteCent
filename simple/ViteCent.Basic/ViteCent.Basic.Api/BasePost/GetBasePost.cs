...
public class GetBasePost(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseApi<GetBasePostArgs, DataResult<BasePostResult>>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    // 设置路由名称
    [Route("Get")]
    public override async Task<DataResult<BasePostResult>> InvokeAsync(GetBasePostArgs args)
    {
        // 记录方法调用日志，便于追踪和调试
        ...
        if (user.IsSuper != (int)YesNoEnum.Yes)
            if (string.IsNullOrEmpty(args.CompanyId))
                return new DataResult<BasePostResult>(500, "公司标识不能为空");
