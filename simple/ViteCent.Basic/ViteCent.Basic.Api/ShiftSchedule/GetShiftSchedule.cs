...
public class GetShiftSchedule(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseApi<GetShiftScheduleArgs, DataResult<ShiftScheduleResult>>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 4. 返回查询结果
    /// </remarks>
    /// <param name="args">查询参数，包含获取换班申请所需的条件</param>
    /// <returns>返回包含换班申请的数据结果对象</returns>
    // 标记为POST请求
    [HttpPost]
    ...
    public override async Task<DataResult<ShiftScheduleResult>> InvokeAsync(GetShiftScheduleArgs args)
    {
        ...
        if (check != null && !check.Success)
            return new DataResult<ShiftScheduleResult>(check.Code, check.Message);
