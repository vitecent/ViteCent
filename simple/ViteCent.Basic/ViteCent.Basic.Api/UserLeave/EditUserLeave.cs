...
public partial class EditUserLeave(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseApi<EditUserLeaveArgs, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 5. 返回操作结果
    /// </remarks>
    /// <param name="args">编辑参数，包含需要更新的请假申请数据</param>
    /// <returns>返回基础结果对象，表示操作是否成功</returns>
    // 标记为POST请求
    [HttpPost]
    // 权限验证过滤器，验证用户是否有权限访问该接口
    [TypeFilter(typeof(BaseAuthFilter), Arguments = new object[] { "Basic", "UserLeave", "Edit" })]
    // 设置路由名称
    [Route("Edit")]
    public override async Task<BaseResult> InvokeAsync(EditUserLeaveArgs args)
    {
