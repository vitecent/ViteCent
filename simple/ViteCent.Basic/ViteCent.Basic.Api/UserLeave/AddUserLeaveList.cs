...
public class AddUserLeaveList(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseListApi<List<AddUserLeaveArgs>, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 7. 返回操作结果
    /// </remarks>
    /// <param name="args">批量新增请假申请的参数</param>
    ...
    // 设置路由名称
    [Route("AddList")]
    public override async Task<BaseResult> InvokeAsync(List<AddUserLeaveArgs> args)
    {
        // 记录方法调用日志，便于追踪和调试
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "请假申请不能为", cancellationToken);

            // 返回操作结果
            return new BaseResult(500, "请假申请不能为空");
        }
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "请假申请重复", cancellationToken);

            // 返回操作结果
            return new BaseResult(500, "请假申请重复");
        }
        ...
        // 构建请求参数
        var request = new AddUserLeaveListArgs()
        {
            Items = args
        };
