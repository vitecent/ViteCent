...
public class AddRepairScheduleList(ILogger<AddRepairScheduleList> logger,
    IBaseCache cache,
    IMapper mapper,
    IMediator mediator,
    ...
    IHttpContextAccessor httpContextAccessor) : IRequestHandler<AddRepairScheduleListArgs, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// <returns></returns>
    public async Task<BaseResult> Handle(AddRepairScheduleListArgs request, CancellationToken cancellationToken)
    {
        logger.LogInformation("Invoke ViteCent.Basic.Application.RepairSchedule.AddRepairScheduleList");

        user = httpContextAccessor.InitUser();

        var check = await AddRepairSchedule.OverrideHandle(request, user);
