...
public partial class LastSchedule
{
    ...
    private async Task<BaseResult> OverrideHandle(ScheduleEntity entity, CancellationToken cancellationToken)
    {
        if (!entity.FirstTime.HasValue)
            return new BaseResult(500, "上班时间不存在");

        if (entity.LastTime.HasValue)
            return new BaseResult(500, "下班时间已存在");
    ...
    /// <param name="cancellationToken"></param>
    /// <returns></returns>
    private async Task<BaseResult> OverrideHandle(LastScheduleArgs request, CancellationToken cancellationToken)
    {
