...
public class DeleteSchedule(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : IRequestHandler<DeleteScheduleArgs, BaseResult>
{
    /// <summary>
    /// 处理删除排班信息的请求
    /// </summary>
    /// <param name="request">删除排班信息的请求参数</param>
    ...
    public async Task<BaseResult> Handle(DeleteScheduleArgs request, CancellationToken cancellationToken)
    {
        ...
        // 将删除请求参数映射为查询参数
        var getArgs = mapper.Map<GetScheduleEntityArgs>(request);
        ...
        // 将排班信息模型映射为删除模型参数
        var args = mapper.Map<DeleteScheduleEntity>(entity);

        // 发送删除命令并获取结果
        var result = await mediator.Send(args, cancellationToken);

        // 触发删除事件通知
        await AddSchedule.OverrideTopic(mediator, TopicEnum.Delete, entity, cancellationToken);
