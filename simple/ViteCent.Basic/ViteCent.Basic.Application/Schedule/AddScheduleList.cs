...
public class AddScheduleList(
    ...
    // 继承基类，指定查询参数和返回结果类型
    : IRequestHandler<AddScheduleListArgs, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    public async Task<BaseResult> Handle(AddScheduleListArgs request,
        CancellationToken cancellationToken)
    {
        // 记录方法调用日志，便于追踪和调试
        logger.LogInformation("Invoke ViteCent.Basic.Application.Schedule.AddScheduleList");
        ...
        foreach (var entity in entitys.Items)
            await AddSchedule.OverrideTopic(mediator, TopicEnum.Add, entity, cancellationToken);
