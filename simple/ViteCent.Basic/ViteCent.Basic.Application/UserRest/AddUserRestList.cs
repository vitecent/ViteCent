...
public class AddUserRestList(
    ...
    : IRequestHandler<AddUserRestListArgs, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// <returns></returns>
    public async Task<BaseResult> Handle(AddUserRestListArgs request,
        CancellationToken cancellationToken)
    {
        ...
        var check = await AddUserRest.OverrideHandle(mediator, request, user, companyInvoke, departmentInvoke, userInvoke, cancellationToken);
        ...
        foreach (var entity in entitys.Items)
            await AddUserRest.OverrideTopic(mediator, TopicEnum.Add, entity, cancellationToken);
