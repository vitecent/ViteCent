    ...
    : IRequestHandler<EnableBaseRolePermissionArgs, BaseResult>
{
    ...
    /// 6. 发送启用事件通知
    /// </remarks>
    /// <param name="request">启用角色权限的请求参数</param>
    /// <param name="cancellationToken">取消令牌</param>
    /// <returns>返回操作结果</returns>
    public async Task<BaseResult> Handle(EnableBaseRolePermissionArgs request, CancellationToken cancellationToken)
    {
        // 记录方法调用日志，便于追踪和调试
        logger.LogInformation("Invoke ViteCent.Auth.Application.BaseRolePermission.EnableBaseRolePermission");
        ...
        if (entity.Status == (int)StatusEnum.Enable)
            return new BaseResult(500, "角色权限已启用");
