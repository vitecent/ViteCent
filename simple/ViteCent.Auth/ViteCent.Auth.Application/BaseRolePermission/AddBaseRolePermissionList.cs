...
public class AddBaseRolePermissionList(
    ...
    // 继承基类，指定查询参数和返回结果类型
    : IRequestHandler<AddBaseRolePermissionListArgs, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// <returns></returns>
    public async Task<BaseResult> Handle(AddBaseRolePermissionListArgs request,
        CancellationToken cancellationToken)
    {
        ...
        foreach (var entity in entitys.Items)
            await AddBaseRolePermission.OverrideTopic(mediator, TopicEnum.Add, entity, cancellationToken);
