...
public class GetBaseOperation(ILogger<GetBaseOperation> logger, IMapper mapper, IMediator mediator, IHttpContextAccessor httpContextAccessor) : IRequestHandler<GetBaseOperationArgs, DataResult<BaseOperationResult>>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    public async Task<DataResult<BaseOperationResult>> Handle(GetBaseOperationArgs request, CancellationToken cancellationToken)
    {
        ...
        if (entity == null)
            return new DataResult<BaseOperationResult>(500, "数据不存在或无权限");
