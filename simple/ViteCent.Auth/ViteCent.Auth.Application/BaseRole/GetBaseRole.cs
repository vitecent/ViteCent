...
public class GetBaseRole(ILogger<GetBaseRole> logger, IMapper mapper, IMediator mediator, IHttpContextAccessor httpContextAccessor) : IRequestHandler<GetBaseRoleArgs, DataResult<BaseRoleResult>>
{
    /// <summary>
    /// </summary>
    ...
    public async Task<DataResult<BaseRoleResult>> Handle(GetBaseRoleArgs request, CancellationToken cancellationToken)
    {
        ...
        if (entity == null)
            return new DataResult<BaseRoleResult>(500, "数据不存在或无权限");
