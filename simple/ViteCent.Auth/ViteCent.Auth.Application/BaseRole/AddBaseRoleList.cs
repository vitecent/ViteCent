    ...
    IHttpContextAccessor httpContextAccessor) : IRequestHandler<AddBaseRoleListArgs, BaseResult>
{
    ...
    public async Task<BaseResult> Handle(AddBaseRoleListArgs request, CancellationToken cancellationToken)
    {
        ...
        var check = await AddBaseRole.OverrideHandle(mediator, request, user, cancellationToken);
        ...
        foreach (var entity in entitys.Items)
            await AddBaseRole.OverrideTopic(mediator, TopicEnum.Add, entity, cancellationToken);
