...
public class AddBaseSystemList(ILogger<AddBaseSystemList> logger,
    ...
    IMapper mapper,
    IMediator mediator,
    IHttpContextAccessor httpContextAccessor) : IRequestHandler<AddBaseSystemListArgs, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// <returns></returns>
    public async Task<BaseResult> Handle(AddBaseSystemListArgs request, CancellationToken cancellationToken)
    {
        logger.LogInformation("Invoke ViteCent.Auth.Application.BaseSystem.AddBaseSystemList");

        user = httpContextAccessor.InitUser();

        var check = await AddBaseSystem.OverrideHandle(mediator, request, user);
