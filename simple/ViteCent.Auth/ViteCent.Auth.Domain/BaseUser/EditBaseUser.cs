...
/// <summary>
/// 编辑用户信息领域服务类 </summary> <remarks> 该类负责处理编辑用户信息的业务逻辑，包括：
/// 1. 记录操作日志
/// 2. 调用基类方法更新用户信息
/// 3. 返回操作结果 </remarks> <param name="logger">日志记录器实例</param>
public class EditBaseUser(
    // 注入日志记录器
    ILogger<EditBaseUser> logger)
    : BaseDomain<BaseUserEntity>, IRequestHandler<BaseUserEntity, BaseResult>
{
    /// <summary>
    /// 数据库名称
    /// </summary>
    ...
    /// <summary>
    /// 处理编辑用户信息的请求 </summary> <param name="request">包含更新信息的用户信息模型</param> <param
