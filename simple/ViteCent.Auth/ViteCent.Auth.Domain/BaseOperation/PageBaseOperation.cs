...
/// </summary>
/// <remarks>
/// 该类负责处理操作信息的分页查询请求，主要功能包括：
/// 1. 接收并处理分页查询参数
/// 2. 调用基础设施层执行分页查询
/// 3. 返回符合条件的操作信息列表
/// </remarks>
/// <param name="logger">日志记录器，用于记录处理过程中的关键信息</param>
public class PageBaseOperation(
    // 注入日志记录器
    ILogger<PageBaseOperation> logger) 
    // 继承基类，指定查询参数和返回结果类型
    : BaseDomain<BaseOperationEntity>, IRequestHandler<SearchBaseOperationEntityArgs, List<BaseOperationEntity>>
{
    /// <summary>
    /// 数据库名称
    /// </summary>
