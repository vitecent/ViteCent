...
public class AddBaseSystemList(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseListApi<List<AddBaseSystemArgs>, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 7. 返回操作结果
    /// </remarks>
    /// <param name="args">批量新增系统信息的参数</param>
    ...
    // 设置路由名称
    [Route("AddList")]
    public override async Task<BaseResult> InvokeAsync(List<AddBaseSystemArgs> args)
    {
        // 记录方法调用日志，便于追踪和调试
        ...
        var logsArgs = new AddBaseLogsArgs()
        {
            ...
            ResourceName = "BaseSystem",
            OperationId = string.Empty,
            OperationName = "Add",
            Description = "批量新增系统信息",
            Args = args.ToJson()
        };
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "系统信息不能为", cancellationToken);

            // 返回操作结果
            return new BaseResult(500, "系统信息不能为空");
        }
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "系统信息重复", cancellationToken);

            // 返回操作结果
            return new BaseResult(500, "系统信息重复");
        }
        ...
        // 构建请求参数
        var request = new AddBaseSystemListArgs()
        {
            Items = args
        };
