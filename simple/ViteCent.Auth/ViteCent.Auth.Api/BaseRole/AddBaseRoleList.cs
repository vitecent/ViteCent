...
public class AddBaseRoleList(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseListApi<List<AddBaseRoleArgs>, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 7. 返回操作结果
    /// </remarks>
    /// <param name="args">批量新增角色信息的参数</param>
    ...
    // 设置路由名称
    [Route("AddList")]
    public override async Task<BaseResult> InvokeAsync(List<AddBaseRoleArgs> args)
    {
        // 记录方法调用日志，便于追踪和调试
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "角色信息不能为", cancellationToken);

            // 返回操作结果
            return new BaseResult(500, "角色信息不能为空");
        }
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "角色信息重复", cancellationToken);

            // 返回操作结果
            return new BaseResult(500, "角色信息重复");
        }
        ...
        // 构建请求参数
        var request = new AddBaseRoleListArgs()
        {
            Items = args
        };
