...
public class AddBaseRolePermissionList(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseListApi<List<AddBaseRolePermissionArgs>, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 7. 返回操作结果
    /// </remarks>
    /// <param name="args">批量新增角色权限的参数</param>
    ...
    // 设置路由名称
    [Route("AddList")]
    public override async Task<BaseResult> InvokeAsync(List<AddBaseRolePermissionArgs> args)
    {
        // 记录方法调用日志，便于追踪和调试
        ...
        // 构建请求参数
        var request = new AddBaseRolePermissionListArgs()
        {
            Items = args
        };
