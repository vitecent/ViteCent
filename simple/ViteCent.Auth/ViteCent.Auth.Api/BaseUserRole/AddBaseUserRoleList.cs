...
public class AddBaseUserRoleList(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseListApi<List<AddBaseUserRoleArgs>, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    // 设置路由名称
    [Route("AddList")]
    public override async Task<BaseResult> InvokeAsync(List<AddBaseUserRoleArgs> args)
    {
        // 记录方法调用日志，便于追踪和调试
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "用户角色不能为", cancellationToken);
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "用户角色重复", cancellationToken);
        ...
        // 构建请求参数
        var request = new AddBaseUserRoleListArgs()
        {
            Items = args
        };
