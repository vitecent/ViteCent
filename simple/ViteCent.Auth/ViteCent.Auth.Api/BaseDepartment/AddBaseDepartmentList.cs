...
public class AddBaseDepartmentList(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseListApi<List<AddBaseDepartmentArgs>, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 7. 返回操作结果
    /// </remarks>
    /// <param name="args">批量新增部门信息的参数</param>
    ...
    // 设置路由名称
    [Route("AddList")]
    public override async Task<BaseResult> InvokeAsync(List<AddBaseDepartmentArgs> args)
    {
        // 记录方法调用日志，便于追踪和调试
        logger.LogInformation("Invoke ViteCent.Auth.Api.BaseDepartment.AddBaseDepartmentList");
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "部门信息不能为", cancellationToken);

            // 返回操作结果
            return new BaseResult(500, "部门信息不能为空");
        }
        ...
        {
            // 记录失败操作日志
            await mediator.LogError(logsArgs, "部门信息重复", cancellationToken);
        ...
        {
            // 重写调用方法
            AddBaseDepartment.OverrideInvoke(item, user);
        ...
        // 构建请求参数
        var request = new AddBaseDepartmentListArgs()
        {
            Items = args
        };
