...
public class GetBaseDepartment(
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseApi<GetBaseDepartmentArgs, DataResult<BaseDepartmentResult>>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 4. 返回查询结果
    /// </remarks>
    /// <param name="args">查询参数，包含获取部门信息所需的条件</param>
    /// <returns>返回包含部门信息的数据结果对象</returns>
    // 标记为POST请求
    [HttpPost]
    ...
    public override async Task<DataResult<BaseDepartmentResult>> InvokeAsync(GetBaseDepartmentArgs args)
    {
        ...
        if (user.IsSuper != (int)YesNoEnum.Yes)
            if (string.IsNullOrEmpty(args.CompanyId))
                return new DataResult<BaseDepartmentResult>(500, "公司标识不能为空");
