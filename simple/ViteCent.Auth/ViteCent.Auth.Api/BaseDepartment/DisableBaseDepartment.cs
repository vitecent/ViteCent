...
public class DisableBaseDepartment(
    // 注入日志记录器
    ...
    IMediator mediator)
    // 继承基类，指定查询参数和返回结果类型
    : BaseApi<DisableBaseDepartmentArgs, BaseResult>
{
    /// <summary>
    /// 用户信息
    /// </summary>
    ...
    /// 4. 返回操作结果
    /// </remarks>
    /// <param name="args">禁用参数，包含需要禁用的部门信息标识</param>
    /// <returns>返回基础结果对象，表示操作是否成功</returns>
     // 标记为POST请求
    [HttpPost]
    // 权限验证过滤器，验证用户是否有权限访问该接口
    [TypeFilter(typeof(BaseAuthFilter), Arguments = new object[] { "Auth", "BaseDepartment", "Disable" })]
    // 设置路由名称
    [Route("Disable")]
    public override async Task<BaseResult> InvokeAsync(DisableBaseDepartmentArgs args)
    {
        // 记录方法调用日志，便于追踪和调试，便于追踪和调试
        logger.LogInformation("Invoke ViteCent.Auth.Api.BaseDepartment.DisableBaseDepartment");
