...
public partial class AddBaseDatabase
{
    ...
    internal static async Task<BaseResult> OverrideHandle(IMediator mediator,
        AddBaseDatabaseListArgs request,
        BaseUserInfo user,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var hasListArgs = new HasBaseDatabaseEntityListArgs
        {
    ...
    /// <param name="cancellationToken">取消令牌</param>
    /// <returns>处理结果</returns>
    private async Task<BaseResult> OverrideHandle(AddBaseDatabaseArgs request,
        CancellationToken cancellationToken)
    {
        var companyId = user?.Company?.Id ?? string.Empty;
