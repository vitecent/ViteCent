    ...
    : BaseApi<EditBaseLogsArgs, BaseResult>
{
    ...
    [TypeFilter(typeof(BaseAuthFilter), Arguments = new object[] { "Basic", "BaseLogs", "Edit" })]
    // 设置路由名称
    [Route("Edit")]
    public override async Task<BaseResult> InvokeAsync(EditBaseLogsArgs args)
    {
        // 记录方法调用日志，便于追踪和调试，便于追踪和调试
        logger.LogInformation("Invoke ViteCent.Basic.Api.BaseLogs.EditBaseLogs");
